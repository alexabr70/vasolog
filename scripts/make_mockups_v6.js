...
/**
 * VasoLog Premium Mockup Generator v6 (May 2026)
 *
 * Опция C - полный рефреш скриншотов под Google Play.
 *
 * Изменения v5 → v6:
 *  - Использует headlines_v3.json (native fixes + strategic positioning)
 *  - Output: D:/DEV/vasolog/release/v1.1.3/store_assets/google_play/mockups_v5
 *  - Финский 06_report использует en raw (Flutter overflow workaround)
 */
...
const DATA = JSON.parse(fs.readFileSync(path.join(__dirname, 'headlines_v3.json'), 'utf-8'));
...
async function main() {
  ...
  const baseOut = 'D:/DEV/vasolog/release/v1.1.3/store_assets/google_play/mockups_v5';
  ...
  console.log('=== VasoLog Mockup Generator v6 (Google Play refresh) ===');
  console.log(`Headlines: headlines_v3.json (strategic positioning + native fixes)`);
  ...
  for (const lang of langs) {
    ...
    for (const screen of SCREENS) {
      // Финский 06_report: Flutter overflow в raw - используем en raw
      let rawPath = path.join(baseRaw, lang, `${screen}.png`);
      if (lang === 'fi' && screen === '06_report') {
        rawPath = path.join(baseRaw, 'en', `${screen}.png`);
      }
...
function drawMedicalEmoji(ctx) {
  ...
  ctx.textAlign = 'right';
  ctx.fillText('\u{1FA7A}', CANVAS_W - EMOJI_PADDING, EMOJI_PADDING + EMOJI_SIZE * 0.75);
  ctx.restore();
}
